...
def train_two(model_list, train_loader, optimizer_list, loss_ce, epoch, path_save_info_list):
    ...
    for i in range(len(model_list)):
        model_list[i].train()
    train_epoch_cost = 0; train_T1_ce_cost = 0; train_T2_ce_cost = 0; train_Hol_ce_cost = 0
    ...
    train_Hol_out_stack = []; train_Hol_label_stack = []

    [path_save_info, T1_path_save_info, T2_path_save_info, Hol_path_save_info] = path_save_info_list

    for i, loader in enumerate(train_loader):
        T1_train_Node_list, T1_train_A_list, \
        T2_train_Node_list, T2_train_A_list, \
        Hol_train_Node_list, Hol_train_A_list, train_label = loader
        ...
        _, _, _, Hol_logits = model_list[2](x=Hol_train_Node_list, A=Hol_train_A_list)

        T1_ce_loss = loss_ce(T1_logits, train_label)
        T2_ce_loss = loss_ce(T2_logits, train_label)
        Hol_ce_loss = loss_ce(Hol_logits, train_label)

        loss = T1_ce_loss + T2_ce_loss + Hol_ce_loss

        train_T1_ce_cost += T1_ce_loss.item()
        train_T2_ce_cost += T2_ce_loss.item()
        train_Hol_ce_cost += Hol_ce_loss.item()
        ...
        Hol_prob = torch.softmax(Hol_logits, dim=1)
        train_out_stack.extend(np.argmax(((T1_prob+T2_prob+Hol_prob)/3).cpu().detach().tolist(), axis=1))
        ...
        train_Hol_label_stack.extend(train_label.cpu().detach().tolist())
        train_Hol_out_stack.extend(np.argmax(Hol_prob.cpu().detach().tolist(), axis=1))
    ...
    Hol_f1 = f1_score(train_Hol_label_stack, train_Hol_out_stack)
    ...
    if epoch % 10 == 0:
        ...
        print("[Train    T2]  [Epoch:{}] [Time:{:.1f}] [Loss C:{:.4f}] [ACC C:{:.4f}] [SEN C:{:.4f}] [SPEC C:{:.4f}] [F1 C:{:.4f}]".
              format(epoch, time.time() - start_time, train_T2_ce_cost / len(train_loader),
                     ...
                     T2_f1))
        print("[Train   Hol]  [Epoch:{}] [Time:{:.1f}] [Loss C:{:.4f}] [ACC C:{:.4f}] [SEN C:{:.4f}] [SPEC C:{:.4f}] [F1 C:{:.4f}]".
              format(epoch, time.time() - start_time, train_Hol_ce_cost / len(train_loader),
                     accuracy(train_Hol_out_stack, train_Hol_label_stack),
                     sensitivity(train_Hol_out_stack, train_Hol_label_stack),
                     specificity(train_Hol_out_stack, train_Hol_label_stack),
                     Hol_f1))
    ...
    with open(T2_path_save_info.replace(".csv", "_train.csv"), "a") as f:
        f.write(
            "{:},{:},{:},{:},{:},{:}\n".format(epoch, train_T2_ce_cost / len(train_loader),
                                       ...
                                       specificity(train_T2_out_stack, train_T2_label_stack),
                                       T2_f1))
    with open(Hol_path_save_info.replace(".csv", "_train.csv"), "a") as f:
        f.write(
            "{:},{:},{:},{:},{:},{:}\n".format(epoch, train_Hol_ce_cost / len(train_loader),
                                       accuracy(train_Hol_out_stack, train_Hol_label_stack),
                                       sensitivity(train_Hol_out_stack, train_Hol_label_stack),
                                       specificity(train_Hol_out_stack, train_Hol_label_stack),
                                       Hol_f1))
...
def test_two(model_list, test_loader, loss_ce, epoch, path_save_info_list):
    start_time = time.time()
    test_epoch_cost = 0; test_T1_ce_cost = 0; test_T2_ce_cost = 0; test_Hol_ce_cost = 0
    test_out_stack = []; test_label_stack = []

    # branch network evaluation
    test_T1_out_stack = []; test_T1_label_stack = []
    test_T2_out_stack = []; test_T2_label_stack = []
    test_Hol_out_stack = []; test_Hol_label_stack = []
    ...
    with torch.no_grad():
        ...
        for i, loader in enumerate(test_loader):
            ...
            Hol_test_Node_list, Hol_test_A_list, test_label = loader
            ...
            Hol_x1, Hol_x2, Hol_out_features, Hol_logits = model_list[2](x=Hol_test_Node_list, A=Hol_test_A_list)

            T1_ce_loss = loss_ce(T1_logits, test_label)
            T2_ce_loss = loss_ce(T2_logits, test_label)
            Hol_ce_loss = loss_ce(Hol_logits, test_label)

            loss = T1_ce_loss + T2_ce_loss + Hol_ce_loss

            test_T1_ce_cost += T1_ce_loss.item()
            test_T2_ce_cost += T2_ce_loss.item()
            test_Hol_ce_cost += Hol_ce_loss.item()
            ...
            T2_prob = torch.softmax(T2_logits, dim=1)
            Hol_prob = torch.softmax(Hol_logits, dim=1)
            test_out_stack.extend(np.argmax(((T1_prob + T2_prob + Hol_prob) / 3).cpu().detach().tolist(), axis=1))
            ...
            test_Hol_label_stack.extend(test_label.cpu().detach().tolist())
            test_Hol_out_stack.extend(np.argmax(Hol_prob.cpu().detach().tolist(), axis=1))
        ...
        Hol_f1 = f1_score(test_Hol_label_stack, test_Hol_out_stack)
        ...
        if epoch % 10 == 0:
            ...
            print("[Test     T2]  [Epoch:{}] [Time:{:.1f}] [Loss C:{:.4f}] [ACC C:{:.4f}] [SEN C:{:.4f}] [SPEC C:{:.4f}] [F1 C:{:.4f}]".
                  format(epoch, time.time() - start_time, test_T2_ce_cost / len(test_loader),
                         ...
                         T2_f1))
            print("[Test    Hol]  [Epoch:{}] [Time:{:.1f}] [Loss C:{:.4f}] [ACC C:{:.4f}] [SEN C:{:.4f}] [SPEC C:{:.4f}] [F1 C:{:.4f}]".
                  format(epoch, time.time() - start_time, test_Hol_ce_cost / len(test_loader),
                         accuracy(test_Hol_out_stack, test_Hol_label_stack),
                         sensitivity(test_Hol_out_stack, test_Hol_label_stack),
                         specificity(test_Hol_out_stack, test_Hol_label_stack),
                         Hol_f1))
        ...
        with open(Hol_path_save_info.replace(".csv", "_test.csv"), "a") as f:
            f.write(
                "{:},{:},{:},{:},{:},{:}\n".format(epoch, test_Hol_ce_cost / len(test_loader),
                                               accuracy(test_Hol_out_stack, test_Hol_label_stack),
                                               sensitivity(test_Hol_out_stack, test_Hol_label_stack),
                                               specificity(test_Hol_out_stack, test_Hol_label_stack),
                                               Hol_f1))
...
def train_three(model_list, train_loader, optimizer_list, loss_ce, epoch, path_save_info_list):
    ...
    for i in range(len(model_list)):
        model_list[i].train()
    train_epoch_cost = 0; train_T1_ce_cost = 0; train_T2_ce_cost = 0; train_T3_ce_cost = 0; train_Hol_ce_cost = 0
    train_out_stack = []; train_label_stack = []
    ...
    [path_save_info, T1_path_save_info, T2_path_save_info, T3_path_save_info, Hol_path_save_info] = path_save_info_list
    ...
    for i, loader in enumerate(train_loader):
        ...
        _, _, _, Hol_logits = model_list[3](x=Hol_train_Node_list, A=Hol_train_A_list)
        ...
        loss = T1_ce_loss + T2_ce_loss + T3_ce_loss + Hol_ce_loss
        ...
        Hol_prob = torch.softmax(Hol_logits, dim=1)
        train_out_stack.extend(np.argmax(((T1_prob+T2_prob+T3_prob+Hol_prob)/4).cpu().detach().tolist(), axis=1))
...
def test_three(model_list, test_loader, loss_ce, epoch, path_save_info_list):
    start_time = time.time()
    test_epoch_cost = 0; test_T1_ce_cost = 0; test_T2_ce_cost = 0; test_T3_ce_cost = 0; test_Hol_ce_cost = 0
    test_out_stack = []; test_label_stack = []
    ...
    with torch.no_grad():
        ...
        for i, loader in enumerate(test_loader):
            ...
            _, _, _, Hol_logits = model_list[3](x=Hol_test_Node_list, A=Hol_test_A_list)
            ...
            T3_prob = torch.softmax(T3_logits, dim=1)
            Hol_prob = torch.softmax(Hol_logits, dim=1)
            test_out_stack.extend(np.argmax(((T1_prob + T2_prob + T3_prob + Hol_prob) / 4).cpu().detach().tolist(), axis=1))
        ...
        with open(T3_path_save_info, "a") as f:
            f.write(
                "{:},{:},{:},{:},{:},{:}\n".format(epoch, test_T3_ce_cost / len(test_loader),
                                               ...
                                               specificity(test_T3_out_stack, test_T3_label_stack),
                                               T3_f1))
        with open(Hol_path_save_info.replace(".csv", f"_test.csv"), "a") as f:
            f.write(
                "{:},{:},{:},{:},{:},{:}\n".format(epoch, test_Hol_ce_cost / len(test_loader),
                                               accuracy(test_Hol_out_stack, test_Hol_label_stack),
                                               sensitivity(test_Hol_out_stack, test_Hol_label_stack),
